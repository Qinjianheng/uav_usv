...
截击成功或失败后，节点在 `/simulation/impact/result` 发布 `uav_usv_interfaces/InterceptResult`。Evaluator以0.50米三维球作为实验成功判据；planner使用同一个0.50米球，并把接触点放在海面上方 `preferred_clearance`（0.29米）处，两个半径分别记录在config和summary中。**这个净空必须大于机体下沉量**：X500起落架最低点在 `base_link` 下方0.227米，所以净空低于约0.23米时计划终点本身就落在水里，把计划飞完必然先触发触海。这里的几何预算很紧：0.50米球里机体已占0.23米，剩下0.27米要同时容纳净空余量和水平收敛误差；取0.29米可保留0.06米机体余量，捕获仍容许 √(0.50²−0.29²)=0.41 米的水平误差，净空再抬高会以更快的速度牺牲捕获率。配置测试 `test_planned_contact_point_stays_above_the_body_contact_threshold` 守护该不变量，同时要求 `preferred_clearance + 目标升沉 <= planned_capture_radius`（0.29 + 0.05 ≤ 0.50）。轨迹的硬性校验上限用 `contact_clearance`（0.27米）而不是接触点高度，两者相差2厘米，避免终点正好压在校验边界上、几毫米的插值下垂被误判成净空违规。触海判据以**机体最低点**而非PX4参考点为准：X500起落架最低点在 `base_link` 下方0.227米，因此当参考点高度达到 `sea_surface_z - body_lower_extent`（当前为 `-0.23` 米）时即判为机体触水，结果为 `FAILURE / SEA_CONTACT`。`InterceptResult.detail` 区分两种触发方式：`BODY_LOWEST_POINT_AT_SEA_SURFACE`（机体刚触水，参考点仍在水面以上）与 `REFERENCE_POINT_BELOW_SEA_SURFACE`（参考点已越海平面，进水更深）。控制侧 `sea_safety` 的 `UNRECOVERABLE` 只是制动权威耗尽的告警，不构成物理触水，二者不可互相替代。SUCCESS、SEA_CONTACT和TIMEOUT都会触发独立Gazebo暂停请求（失败时短时重试），MissionManager同时进入终态并让tracker清除旧MINCO，只保留海面上方的安全保持点。轨迹丢失时tracker不再把PX4位置设定值瞬间回跳到当前机位：回退指令沿用跟踪阶段的加速度限幅，从最后一次已指令参考点连续推进，并在参考点低于 `recovery_clearance`（0.5米）时以有界垂向速度做"刹车到目标"式爬升，参考点绝不越过 `reserve_clearance`（0.07米）。因此丢失终端计划既不会把机体停在净空线以内，也不会在接管瞬间产生位置设定值跳变；`previous_command_velocity` 在回退期间持续更新，重新跟踪时也不会因陈旧时间差产生大跳变。`/simulation/impact/hit` 只表示SUCCESS，不表示是否已经发生终止事件。结果还记录真实最小距离、相对速度、规划/跟踪拒绝原因、接触时间倒计时及按目标距离分桶的运行性能。
