...
截击模式以20 Hz滚动重规划。规划器从连续目标速度观测估计转向率；远距离阶段保持5米巡航高度并持续水平闭合，进入相对移动捕获点的45度几何下滑线后连续俯冲，近距离仍受前视相机最大48.7度目标俯角和海面净空约束。这里的45度是飞行轨迹角，不是强制机体俯仰角。可行轨迹在3米终端区内激活后锁定终端下降；后续重规划暂时不可行时，控制器先沿上一条已验证可行轨迹继续执行其剩余时域。轨迹结束后若目标仍在4米内，则保持终端下降并直接追逐安全捕获高度，不再跳回目标上方；目标重新拉开到4米以外时才释放终端下降。追逐制动点是0.25米实际捕获邻域，而不是3米终端规划边界。进入可达域后在1–2秒范围内搜索末端时间和闭合速度，以三段MINCO-T3五次多项式约束起终点位置、速度与加速度，并执行轨迹上前视0.15秒的速度状态；段间保持到四阶导数连续。候选轨迹只有在全时域满足速度、加速度、捕获半径和海面安全约束时才会执行，不可行时逐级降低目标曲率权重，最终回退直接追逐。观测不足或目标速度过低时目标预测自动退回恒速度模型。
