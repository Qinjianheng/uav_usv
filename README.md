...
截击控制保持20 Hz，计算量较大的MINCO搜索限制为10 Hz，搜索间隔内只短时跟踪上一条轨迹。目标预测采用有界自适应CTRA：由连续速度观测在线估计转率、转率加速度和纵向加速度，并让加速度随预测时域衰减；它不读取目标预设航线，适用于转率和速度均会改变的非合作机动目标，数据不足时自动退回恒速度模型。远距离阶段保持5米巡航高度并持续水平闭合，进入相对移动捕获点的45度几何下滑线后连续俯冲，近距离仍受前视相机最大48.7度目标俯角和捕获兼容的海面净空约束。这里的45度是飞行轨迹角，不是强制机体俯仰角。进入可达域后在1–2秒范围内搜索末端时间和闭合速度，以三段MINCO-T3五次多项式约束起终点状态；执行时使用“轨迹速度+位置误差反馈”，而不是纯速度前馈。重规划失败时旧轨迹最多保留0.2秒，并且预测接触点偏移超过0.5米就立即废弃。所有规划和追逐分支都经过基于实际高度、下降速度、响应时间及制动距离的海面安全屏障；MINCO末端垂向速度不再沿三维闭合方向继续穿越海面。候选轨迹不可行或失效后回退直接追逐，追逐制动点仍是0.25米实际捕获邻域，而不是3米终端规划边界。
